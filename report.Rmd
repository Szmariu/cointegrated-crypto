...
The Etherium plot looks very similar to Bitcoin, although 
...
const. This is due to the changes in variance of the 
underlying variable.   
...
Judging by the BG test, Etherium prices are a I(1) process, so one 
...
Testing the variable differenced once confirms this. BG is not 
...
has significant spikes in price in some places, so nothing really can be done 
about it.   
...
Everything is insignificant, on on the verge of significance, which is good news.   
...
Because the price is so stable, the prediction looks kind of 
