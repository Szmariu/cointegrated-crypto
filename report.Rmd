...
*4.3%*, compared to *3.4%* achived by the VAR model. It was observed 
that for the examined period of time, Bitcoin and Etherium close
...
This study will be conducted on the daily closing prices of two crytpocurrencies - 
Bitcoin and Etherium. Bitcoin was choosen because it is a dominant force on the market, 
Etherium was choosen because it seems to follow Bitcoin price very closely.   

The dates are from 2019-01-01 to today. This gives enought data to estimate the models,
while at the same time reomves the one-of-a-kind huge price surge that happened in 
2018.   

Out of sample period for estimation was choosen to be 30 days. It should be enough
...
The Bitcoin plot looks like this. Significant changes in price in both directions with no 
visible trend, but nothing like 2018.   
...
The Etherium plot looks very simmilar to Bitcoin, aldought 
it seems to have slightly higher variance.   
...
Etherium reacts to changes stronger than Bitcoin. They do hovever 
seem very cointgrated. 
...
This is the same plot, but as a difference beetween prices. 
As it can be seen, when the change is very short and strong, 
the prices move like one. This could be a result of automated 
trading, but more indepth research would be required.   
...
BG test signifies a autocorrelation of order 1, so 
...
After differencing, all BG tests are insignificant. This 
...
Some lags seem to be on the verge of beeing significant. Thanks to the
...
The residuals have a mean of zero, but the vairance is not
const. This is due to the changes in vairance of the 
underlying variable.   

```{r fig.height = 7}
plot(resid(BTC.arima2010.small))
```
Nothing is significant - good. 
...
Jugding by the BG test, Etherium prices are a I(1) process, so one 
...
Testing the variable differenced onec confirms this. BG is not 
significant here for 0 augmentations, which means that there is 
no autocorrelation in the variable.
...
Residuals have mean of 0, but they are not stationary 
because the variance changes. This is due to the original data that 
has significant spikes in pric in some places, so nothing really can be done 
about it.   
...
Everything is insignificant, on on the verge os significance, which is good news.   
...
BJ test confirms that the residuals are not in fact
autocerrelated. 
...
aldought it seems to overestimate the price and ignore the 
downward trend present in the data. 
Becaues the price is so stable, the precistion looks kind of 
...
Simmilar results for Etherium - aldought here it seems that 
...
For VAR, the best number of lags was 7. The VAR model achived better results,
...
for some smaller currencys, they could be used for trading. 
...
Other possibilites could be to use cross validation to test the performance 
of the models on different parts of data (the one choosen was quite calm). 
